...
export async function POST(req: NextRequest) {
  console.log(`/credentials - Credential offer exchange initiated`);

  const token = headers().get("authorization")?.split(" ")[1];

  console.log(`/credentials - Validating authorization header`);

  if (!token) {
    console.error(`/credentials - Authorization header not found`);
  ...
  console.log(
    `/credentials - Pre-authorization_code exchange flow initiated. Code: ${payload.preAuthorizedCode}`
  );
  ...
  console.log(
    `/credentials - Credential offer scanned. Credential Offer: ${JSON.stringify(
      credential_offer
    )}`
  );
  ...
  console.log(`/credentials - Credential JWT signed. JWT VC: ${signedJwt}`);
