...
export interface CredentialOfferDocument extends mongoose.Document {
  code: string;
  status: string;
  type: string;
  data: object;
...
const CredentialOfferDocumentSchema =
  new mongoose.Schema<CredentialOfferDocument>({
    code: {
      type: String,
...
export default mongoose.models.CredentialOfferDocument ||
  mongoose.model<CredentialOfferDocument>(
